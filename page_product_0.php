...
if (isset($_GET['product_id'])) {
    ...
    if ($info_produit) {
        ...
        <head>
            ...
            <title>Apie <?= $info_produit['product_name'] ?></title>
        ...
        <body>
            ...
            <main>
                ...
                <section class="presentation-product">
                    <img src="data:image/png;base64,<?= base64_encode($info_produit['product_img_hero_1']) ?>" alt="<?= $info_produit['product_alt'] ?>" class="img-product">
                    <div class="pre-desc">
                ...
                <section class="list-quality-product">
                    <div class="container-quality-product">
                        <?php foreach ($info_produit['qualities'] as $quality): ?>
                            <?php if (!empty($info_produit['qualities'])) { ?>
                ...
                <section class="info-supp">
                    <div class="pre-fonct">
                        <?php if (!empty($info_produit['product_fonct'])) { ?>
                            <h5 class="subtitle">
                                Fonctionnalites
                            </h5>
                        <?php } ?>
                        <?= $info_produit['product_fonct'] ?>
                    </div>

                    <div class="list-models">
                        <?php
                        // Vérifier si la section modul-pages doit être affichée (par exemple, en vérifiant si des données ont été récupérées de la base de données)
                        if ($info_produit && isset($info_produit['models']) && !empty($info_produit['models'])) {
                            foreach ($info_produit['models'] as $models) {
                                // Récupérer les informations pour chaque modul
                                ...
                                ?>
                                <div class="box-model">
                                    <img src="data:image/png;base64,<?= base64_encode($models['model_img']) ?>" alt="">
                                    <p class="text"><?= $model_text ?></p>
                                </div>
                                <?php
                            }
                        }
